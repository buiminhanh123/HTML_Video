...
import type { TitleSlide } from '../../lib/types';
...
export const TitleScene: React.FC<Props> = ({ slide, username, accentColor, slideStartFrame }) => {
      ...
      <div style={{
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 24, marginTop: 20 }}>
          {slide.bullets.map((bullet, i) => {
            const myDelay = bullet.delay ?? (25 + i * 18);
            const nextBullet = slide.bullets[i + 1];
            const nextDelay = nextBullet?.delay ?? (i + 1 < slide.bullets.length ? 25 + (i + 1) * 18 : slide.durationInFrames);
            return (
